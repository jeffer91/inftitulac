...
- Pedir estudiantes paginados desde BaseLocalAPI, no cortar la lista después de procesarla completa.
Con qué se conecta:
...
(function(window,document){
  ...
  var state = {tab:"periodos",periodId:"",divisionFilter:"",search:"",statusFilter:"ACTIVO",loading:false,dailyStarted:false,renderPending:false,renderTimer:null,lastRenderError:null,lastView:null,studentPage:1,studentPageSize:100,lastDashboard:null,lastEventAt:0,lastEventName:""};
  ...
  function invalidateCaches(){safeCall("BL2DashboardRepo.invalidate", function(){if(window.BL2DashboardRepo && typeof window.BL2DashboardRepo.invalidate === "function"){window.BL2DashboardRepo.invalidate();}}, null);safeCall("BaseLocalAPI.clearSnapshotCache", function(){if(window.BaseLocalAPI && typeof window.BaseLocalAPI.clearSnapshotCache === "function"){window.BaseLocalAPI.clearSnapshotCache();}}, null);}
  ...
  function setBusy(isBusy, message, mode){
    ...
    var map = [["bl-btn-pull-firebase", mode === "pull" ? "Bajando..." : "Solo bajar Firebase"],["bl-btn-sync-now", mode === "sync" ? "Sincronizando..." : "Sincronizar ahora"],["bl-btn-clean-base", mode === "clean" ? "Limpiando..." : "Limpiar base"],["bl-btn-delete-period", mode === "delete" ? "Borrando..." : "Borrar período"]];
    map.forEach(function(item){var btn=el(item[0]);if(btn){btn.disabled=!!isBusy;btn.textContent=isBusy?item[1]:item[1];}});
  ...
  function renderStudentJson(row){return JSON.stringify(Object.assign({}, row || {}), null, 2);}
  function bindStudentInspectButtons(rows){
    rows = rows || [];
    document.querySelectorAll("[data-bl-student-json]").forEach(function(button){
      button.addEventListener("click", function(){
        var index = Number(button.getAttribute("data-bl-student-json"));var row = rows[index] || null;
        if(!row){status("No encontré el registro local de ese estudiante.", "bl-status-warn");return;}
        var json = renderStudentJson(row);
        try{if(navigator.clipboard && navigator.clipboard.writeText){navigator.clipboard.writeText(json).catch(function(){});}}catch(error){}
        var win = window.open("", "_blank", "width=900,height=700");
        if(win && win.document){win.document.write('<!doctype html><html><head><meta charset="utf-8"><title>Registro Base Local</title><style>body{font-family:Consolas,monospace;padding:18px;background:#f8fafc;color:#0f172a;}pre{white-space:pre-wrap;background:white;border:1px solid #dbe3ef;border-radius:12px;padding:16px;}</style></head><body><h3>Registro guardado en Base Local</h3><pre>' + esc(json) + '</pre></body></html>');win.document.close();}
        else{window.prompt("Registro guardado en Base Local", json);}
      });
    });
  }
  ...
  function renderStudents(view){
    ...
    var headers=["Cédula","Nombre","Carrera","División","Sede","Estado","Período"].concat(REQ_COLUMNS.map(function(req){return req.label;})).concat(["Guardado local"]);
  ...
  function render(){
    ...
    try{
      ...
      if(!state.loading){status("Base Local cargada en modo rápido. Pestaña: "+state.tab+". División: "+(state.divisionFilter||"Todas")+". Registros visibles: "+((view.students||[]).length)+" de "+(view.totalStudentsVisible||0)+".","bl-status-ok");}
  ...
  function scheduleRender(reason, delay){if(state.renderTimer){clearTimeout(state.renderTimer);}state.renderPending=true;state.renderTimer=setTimeout(function(){state.renderTimer=null;render(reason||"programado");}, delay == null ? 220 : delay);}
  ...
  function boot(){
    ...
    if(el("bl-filter-search")){el("bl-filter-search").addEventListener("input",function(event){state.search=event.target.value;state.studentPage=1;scheduleRender("search",420);});}
