...
- Centralizar tablas, claves, normalización e índices para consultas rápidas.
- Evitar cargas pesadas y normalizaciones repetidas por pantalla.
- Preparar consultas por período, cédula, carrera, sede, jornada, matrícula y cumplimiento.
Con qué se conecta:
- bl2-storage.js
...
(function(window){
  ...
  INDEXES[STORES.estudiantes.name] = [
    ...
    {name:"by_idLocal", keyPath:"idLocal", options:{unique:false}},
    ...
    {name:"by_cedula_periodo", keyPath:["numeroIdentificacion","periodoId"], options:{unique:false}},
    {name:"by_updatedAt", keyPath:"updatedAt", options:{unique:false}}
  ];
  ...
  function firstValue(row, names, fallback){
    ...
    for(var i = 0; i < names.length; i += 1){
      if(row[names[i]] != null && text(row[names[i]]) !== ""){return row[names[i]];}
    }
    return fallback;
  }

  function cedulaOf(row){
    return text(firstValue(row, [
      "cedula", "Cedula", "Cédula", "CEDULA",
      "numeroIdentificacion", "numeroidentificacion", "NumeroIdentificacion", "NúmeroIdentificación",
  ...
  function estadoOf(row){
    var raw = searchKey(firstValue(row, ["estadoMatricula", "EstadoMatricula", "estado", "Estado"], "ACTIVO")).toUpperCase();
    if(raw.indexOf("RETIR") >= 0){return "RETIRADO";}
    if(raw.indexOf("INACT") >= 0){return "INACTIVO";}
    return "ACTIVO";
  }

  function cumpleValue(value){
    var raw = searchKey(value).toUpperCase();
    if(!raw){return null;}
    if(raw === "CUMPLE" || raw === "SI" || raw === "APROBADO" || raw === "OK"){return true;}
    if(raw.indexOf("NO CUMPLE") >= 0 || raw === "NO" || raw === "REPROBADO" || raw === "PENDIENTE"){return false;}
    return null;
  }
  ...
  function requirementSummary(row){
    ...
    REQUIREMENT_FIELDS.forEach(function(field){
      ...
      var value = cumpleValue(row[field]);
      if(value == null){return;}
      total += 1;
      if(value){cumple += 1;}
      else{
        noCumple += 1;
        pendientes.push(field);
      }
    });
    ...
    return {
      ...
      porcentaje:total ? Math.round((cumple * 10000) / total) / 100 : 0,
      cumpleGeneral:total > 0 && noCumple === 0
    };
  }

  function compositeStudentId(cedula, periodoId){
    var c = key(cedula) || "sin_cedula";
    var p = key(periodoId) || "sin_periodo";
    return c + "__" + p;
  }
  ...
  function normalizeStudent(row){
    ...
    source.idLocal = text(source.idLocal || source._bl2IdLocal || compositeStudentId(cedula, periodoId));
    ...
    source.searchText = searchKey([
      ...
      source.periodoLabel,
      source.periodoId,
      source.estadoMatricula,
      source.cumpleGeneral ? "cumple" : "no cumple"
    ].join(" "));
  ...
  function normalizePeriod(row){
    var source = row && typeof row === "object" ? Object.assign({}, row) : {};
    var label = text(firstValue(source, ["label", "periodoLabel", "periodo", "Periodo", "nombrePeriodo", "id"], "SIN PERIODO"));
    var periodoId = text(firstValue(source, ["id", "periodoId"], key(label)));
    ...
    source.activo = source.activo === false ? false : true;
    source.updatedAt = text(source.updatedAt || source.actualizadoEn || source.creadoEn) || now();
    source.searchText = searchKey([source.id, source.periodoId, source.label, source.periodoLabel].join(" "));
    return source;
  }
  ...
  window.BL2Schema = {
    ...
    stores:STORES,
    indexes:INDEXES,
    requirementFields:REQUIREMENT_FIELDS.slice(),
    helpers:{
      text:text,
      key:key,
      searchKey:searchKey,
      ...
      cedulaOf:cedulaOf,
      estadoOf:estadoOf,
      cumpleValue:cumpleValue,
      requirementSummary:requirementSummary,
      compositeStudentId:compositeStudentId,
