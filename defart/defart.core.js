...
- Usar defart.divisions.js para leer división -> carreras.
- Bloquear N-DEF si N-ART es menor a 7.
- Limpiar N-DEF y N-FIN cuando N-ART es menor a 7.
- Ignorar Titulación como requisito bloqueante para Defensas/Artículo.
- Calcular N-FIN con fórmula institucional: (N-ART * 0.70) + (N-DEF * 0.30).
========================================================= */
(function(window){
  "use strict";

  var VERSION = "2.3.0-defart-core-divisiones-notas-seguras";
  var STATES = ["Falta requisitos", "Pendiente N-ART", "Supletorio Art", "Pendiente N-DEF", "Supletorio Def", "Aprobado"];
  ...
  var REQUIREMENTS = [
    ...
    { key:"titulacion", label:"Titulación", applies:"ignore", aliases:["titulacion","titulación","Titulacion","Titulación","reqTitulacion","requisitoTitulacion","estadoTitulacion"] }
  ];
  ...
  function divisionsService(){ return window.DefartDivisions || null; }
  ...
  function noteToText(value){
    if(notasService() && typeof notasService().formatearNota === "function"){
      return notasService().formatearNota(value);
    }
  ...
  function isValidNote(value){
    if(notasService() && typeof notasService().validarNota === "function"){
      return notasService().validarNota(value);
    }
  ...
  function careerKey(value){
    if(divisionsService() && typeof divisionsService().careerKey === "function"){
      return divisionsService().careerKey(value);
    }

    if(window.BDLNormCarrera && typeof window.BDLNormCarrera.normalize === "function"){
      try{ return text(window.BDLNormCarrera.normalize(value || "", "").key); }catch(error){}
    }
  ...
  function sameCareer(a, b){
    ...
    if(divisionsService() && typeof divisionsService().sameCareer === "function"){
      return divisionsService().sameCareer(a, b);
    }

    return careerKey(a) === careerKey(b) || norm(a) === norm(b) || compact(a) === compact(b);
  }
  ...
  function appliesRequirement(req, row){
    if(!req){ return false; }
    if(req.applies === "ignore" || req.applies === "never" || req.key === "titulacion"){ return false; }
  ...
  function divisionOf(row){
    if(divisionsService() && typeof divisionsService().divisionOf === "function"){
      return divisionsService().divisionOf(row || {});
    }
    ...
    return list[0] || row.divisionPrincipal || row.division || row.Division || row["División"] || "Sin división";
  }

  function hasDivision(row, division){
    if(!text(division)){ return true; }

    if(divisionsService() && typeof divisionsService().hasDivision === "function"){
      return divisionsService().hasDivision(row || {}, division);
    }
  ...
  function notesOf(row){
    ...
    if(notasService() && typeof notasService().extraerNotas === "function"){
      ...
      nart = numberValue(central.nart);
      ndef = numberValue(central.ndef);
      nfin = numberValue(central.nfin);
    }else{
      nart = numberValue(pick(row, ["Notart", "Nart", "N_ART", "N-ART", "notart", "notaArticulo", "nota_articulo"]));
      ndef = numberValue(pick(row, ["Notdef", "Ndef", "N_DEF", "N-DEF", "notdef", "notaDefensa", "nota_defensa"]));
      nfin = numberValue(pick(row, ["Notafinal", "NotaFinal", "Nfin", "nfin", "N_FIN", "N-FIN", "notaFinal"]));
    }
  ...
  function resolveEstado(req, nart, ndef){
    ...
    if(nart === null){ return "Pendiente N-ART"; }
    if(nart < 7){ return "Supletorio Art"; }
    if(ndef === null){ return "Pendiente N-DEF"; }
    if(ndef < 7){ return "Supletorio Def"; }
  ...
  function preview(row, patch){
    ...
    if(Object.prototype.hasOwnProperty.call(patch, "nart")){
      next.Notart = numberValue(patch.nart);
    }

    if(Object.prototype.hasOwnProperty.call(patch, "ndef")){
      next.Notdef = numberValue(patch.ndef);
    }
  ...
  function periodOptions(rows){
    ...
    rawPeriods().forEach(function(period){
      ...
      if(id){
        map[id] = {
          id:id,
          label:text(p.label || p.periodoLabel || id)
        };
      }
    });
    ...
    rows.forEach(function(row){
      if(row._periodoId && !map[row._periodoId]){
        map[row._periodoId] = {
          id:row._periodoId,
          label:row._periodoLabel || row._periodoId
        };
      }
    });
  ...
  function compareValues(a, b, key){
    ...
    return String(av == null ? "" : av).localeCompare(String(bv == null ? "" : bv), "es", {
      numeric:true,
      sensitivity:"base"
    });
  }
  ...
  function filterRows(options){
    ...
    var rows = rawStudents().filter(isActive).map(decorate).filter(function(row){
      ...
      if(options.career && !sameCareer(row._carrera, options.career)){ return false; }
  ...
  function diagnosticsFromRows(allActive, rows, options){
    ...
    try{
      if(window.RequisitosBL && typeof window.RequisitosBL.conteos === "function"){
        connectorCounts = window.RequisitosBL.conteos();
      }
    }catch(error){}
  ...
  function summary(options){
    ...
    var baseForDivision = allActive.filter(function(row){
      return !options.periodId || samePeriod(row._periodoId, options.periodId);
    });

    var baseForCareer = baseForDivision.filter(function(row){
      return !options.division || hasDivision(row, options.division);
    });

    var divisionList = divisionsService() && typeof divisionsService().divisionList === "function"
      ? divisionsService().divisionList(baseForDivision)
      : unique(baseForDivision, function(row){ return row._division; });

    var careerList = divisionsService() && typeof divisionsService().careerList === "function"
      ? divisionsService().careerList(options.division, baseForDivision)
      : unique(baseForCareer, function(row){ return row._carrera; });

    var diagnostics = diagnosticsFromRows(allActive, rows, options);

    if(divisionsService() && typeof divisionsService().diagnostics === "function"){
      diagnostics.divisions = divisionsService().diagnostics();
    }
  ...
  function findStudentIndex(students, id){
    ...
    for(var i = 0; i < students.length; i += 1){
      ...
      if(studentId(students[i], i) === id || decorated._defId === id || decorated._cedula === id){
        return i;
      }
    }
  ...
  function normalizePatch(change){
    var out = {};

    if(Object.prototype.hasOwnProperty.call(change, "nart")){
      out.Notart = numberValue(change.nart);
    }

    if(Object.prototype.hasOwnProperty.call(change, "ndef")){
      out.Notdef = numberValue(change.ndef);
    }

    return out;
  }

  function validateChange(current, change, nart, ndef){
    var errors = [];
    ...
    if(Object.prototype.hasOwnProperty.call(change, "ndef") && text(change.ndef) !== "" && !isValidNote(change.ndef)){
      errors.push("N-DEF inválida: " + current._nombre);
    }

    if(Object.prototype.hasOwnProperty.call(change, "nart") && !current._canArt){
      errors.push("N-ART bloqueada por falta de requisitos: " + current._nombre);
    }

    if(Object.prototype.hasOwnProperty.call(change, "ndef") && ndef !== null && (!current._canArt || nart === null || nart < 7)){
  ...
  function saveNotes(changes){
    ...
    changes.forEach(function(change){
      ...
      var validationErrors = validateChange(current, change, nart, ndef);
      ...
      var updatedAt = now();
      var notePatch = applyNotesPatch(nart, ndef, {
        updatedAt:updatedAt,
        origen:"defensas"
      });
      ...
      if(current._periodoId){
        savedPeriodMap[current._periodoId] = true;
      }
  ...
  function selfTest(){
    ...
    try{
      snap = getSnapshot();
      check("BaseLocal snapshot", true, "Snapshot leído");
    }catch(error){
      check("BaseLocal snapshot", false, error.message || String(error));
    }
    ...
    check("DefartDivisions", !!window.DefartDivisions, "Lector modular de divisiones");
