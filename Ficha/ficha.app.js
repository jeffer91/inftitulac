...
- Evitar recalcular requisitos/notas para todos los estudiantes de la lista.
- Refrescarse automáticamente cuando BDLocal actualiza el snapshot sin duplicar renders.
...
(function(window, document){
  ...
  var RENDER_DEBOUNCE_MS = 360;
  var REFRESH_DEBOUNCE_MS = 700;
  var DUPLICATE_RENDER_GUARD_MS = 1200;
  ...
  function currentRenderKey(){return [state.periodId, state.division, state.matricula, state.search].join("|");}

  function copyText(value, successMessage){
    if(!window.FichaExport || typeof window.FichaExport.copyText !== "function"){
      status("No se cargó FichaExport.", "warn");
      return Promise.resolve(false);
    }
    return window.FichaExport.copyText(value || "").then(function(){status(successMessage || "Copiado.", "ok");});
  }

  function selectedFromRows(){
    var wanted = text(state.selectedId);
    if(!wanted){return null;}
    return state.rows.find(function(row){return text(row._id) === wanted;}) || null;
  }

  function hydrate(row){
    if(!row){return null;}
    if(window.FichaCore && typeof window.FichaCore.hydrate === "function"){return window.FichaCore.hydrate(row);}
    if(window.FichaCore && typeof window.FichaCore.normalizeStudent === "function"){return window.FichaCore.normalizeStudent(row);}
    return row;
  }
  ...
  function scheduleRender(reason, delay){
    ...
    }, typeof delay === "number" ? delay : RENDER_DEBOUNCE_MS);
  }
  ...
  function fillPeriodAndMatricula(){
    ...
    if(sel && window.FichaCore && typeof window.FichaCore.periods === "function"){
      var list = window.FichaCore.periods().map(periodOptionData);
      ...
      sel.value = state.periodId;
    }
    if(mat){mat.value = state.matricula;}
  }

  function getDivisionOptions(rows){
    var key = [state.periodId, state.matricula, sourceLabel(), rows ? "rows:" + rows.length : "all"].join("|");
    ...
    if(window.FichaCore && typeof window.FichaCore.divisions === "function"){
      state.divisionOptions = window.FichaCore.divisions(rows || null, {periodId:state.periodId, matricula:state.matricula});
    }else{
  ...
  function fillDivisionFilter(rows){
    ...
    if(!div){return;}
    var divisions = getDivisionOptions(rows);
  ...
  function renderList(){
    ...
    box.innerHTML = state.rows.slice(0, 400).map(function(s){
      var active = text(s._id) === text(state.selectedId) ? ' is-active' : '';
  ...
  function renderReqs(row){
    ...
    if(!box || !window.FichaCore){return;}
    var reqs = window.FichaCore.requisitos(row);
    box.innerHTML = reqs.map(function(r){return '<div class="ficha-req"><span class="ficha-req-name">' + esc(r.label) + '</span><span class="ficha-req-value ' + reqClass(r) + '">' + esc(r.estado === "cumple" ? "CUMPLE" : "NO CUMPLE") + '</span></div>';}).join('');
  }
  ...
  function renderNotas(row){
    ...
    box.innerHTML = notas.map(function(n){var ok = n.estado === "cumple";return '<article class="ficha-note ' + (ok ? 'ficha-note-ok' : 'ficha-note-bad') + '"><span>' + esc(n.label) + '</span><strong>' + esc(n.value) + '</strong></article>';}).join('');
  }

  function applySpecialBadge(id, item){
    var node = el(id);
    if(!node || !item){return;}
    var ok = item.estado === "cumple" || item.status === "cumple" || item.cumple === true;
    node.className = "ficha-mini-badge " + (ok ? "ficha-badge-ok" : "ficha-badge-bad");
    node.title = (item.label || "Requisito") + ": " + (ok ? "CUMPLE" : "NO CUMPLE");
    node.setAttribute("aria-label", node.title);
  }

  function renderSpecials(row){
    var list = window.FichaCore && window.FichaCore.especiales ? window.FichaCore.especiales(row) : [];
    var titulacion = list.find(function(x){return x.key === "aprobaciontitulacion";});
    var complexivo = list.find(function(x){return x.key === "aprobacioncomplexivoproyecto";});
  ...
  function renderHeaderIdentity(row){
    ...
    if(!box){return;}
    box.innerHTML = [headerLine("Cédula", row._cedula || "—"), headerLine("Carrera", row._carrera || "Sin carrera"), headerLine("Período", periodDisplay(row))].join("");
  }
  ...
  function renderDetail(row){
    row = hydrate(row);
    if(!row){showEmpty();return;}
    showDetail();
    setText("ficha-nombre", row._nombres || "Sin nombre");
    renderHeaderIdentity(row);
    var estado = el("ficha-estado");
    if(estado){estado.textContent = row._estado && row._estado.label ? row._estado.label : "Pendiente";estado.className = "ficha-pill " + estadoClass(row._estado);}
    ...
    setText("ficha-correo", correoPrincipal(row) || "—");
    ...
    var w = window.FichaCore && window.FichaCore.whatsappUrl ? window.FichaCore.whatsappUrl(row) : "";
    ...
    var tgUrl = window.FichaCore && window.FichaCore.telegramUrl ? window.FichaCore.telegramUrl(row) : "";
  ...
  function updateListActive(){
    ...
    Array.prototype.forEach.call(items, function(item){
      item.classList.toggle("is-active", text(item.getAttribute("data-id")) === wanted);
    });
  }
  ...
  function shouldSkipDuplicate(reason){
    var key = currentRenderKey();
    var duplicateAuto = (reason === "bdlocal-refresh" || reason === "storage" || reason === "programado") && state.lastRenderKey === key && state.rows.length > 0;
    if(duplicateAuto && now() - state.lastRenderAt < DUPLICATE_RENDER_GUARD_MS){return true;}
    return false;
  }

  function render(reason){
    reason = reason || "render";
    if(state.rendering){scheduleRender(reason, RENDER_DEBOUNCE_MS);return;}
    if(shouldSkipDuplicate(reason)){return;}
    state.rendering = true;
    try{
      if(!window.FichaCore || typeof window.FichaCore.filter !== "function"){
        throw new Error("FichaCore no está disponible. Revisa que ficha.core.js cargue antes de ficha.app.js.");
      }

      fillPeriodAndMatricula();

      var force = reason === "bdlocal-refresh" || reason === "refresh";
      ...
      fillDivisionFilter(state.rows);

      if(state.selectedId && !state.rows.some(function(x){return text(x._id) === text(state.selectedId);})){state.selectedId = "";}

      renderList();

      if(state.selectedId){renderDetail(selectedFromRows());}
      else{showEmpty();}

      state.lastRenderKey = currentRenderKey();
      state.lastRenderAt = now();
      state.lastReason = reason;
      status("Ficha lista por " + sourceLabel() + ". Matrícula: " + (state.matricula || "Todos") + ". División: " + (state.division || "Todas") + ". Resultados: " + state.rows.length + ".", "ok");
    }catch(e){
  ...
  function saveModalidad(){
    var row = hydrate(selectedFromRows());
  ...
  function bindBDLocalEvents(){
    ...
    window.addEventListener("storage", function(event){
      if(event && (event.key === "REQ_BDLOCAL_LEGACY_SNAPSHOT_V1" || event.key === "REQ_EXCEL_LOCAL_V1:snapshot" || event.key === "REQ_BL_SIGNAL_V1")){refreshFromBDLocal();}
    });
  ...
  function bind(){
    ...
    bindIf("ficha-search", "input", function(e){state.search = e.target.value;state.selectedId = "";scheduleRender("search");});
    ...
    bindIf("ficha-btn-copy", "click", function(){var row = hydrate(selectedFromRows());if(!row || !window.FichaCore){return;}copyText(window.FichaCore.toText(row), "Ficha copiada.");});
    bindIf("ficha-copy-detail", "click", function(){var row = hydrate(selectedFromRows());if(!row || !window.FichaCore){return;}copyText(window.FichaCore.toText(row), "Ficha copiada.");});
    bindIf("ficha-modalidad-save", "click", saveModalidad);
    bindIf("ficha-telegram", "click", function(e){
      var row = hydrate(selectedFromRows());
      if(!row || !window.FichaCore){return;}
      var url = window.FichaCore.telegramUrl ? window.FichaCore.telegramUrl(row) : "";
      if(!url){e.preventDefault();status("Telegram no registrado.", "warn");return;}
      e.preventDefault();
      copyText(window.FichaCore.studentMessage(row), "Mensaje copiado para Telegram.").then(function(){window.open(url, "_blank", "noopener");});
    });
    bindIf("ficha-copy-cedula", "click", function(){var row = hydrate(selectedFromRows());if(row){copyText(row._cedula, "Cédula copiada.");}});
    bindIf("ficha-copy-correo", "click", function(){var row = hydrate(selectedFromRows());if(row){copyText(correoPrincipal(row), "Correo copiado.");}});
    bindBDLocalEvents();
  }
  ...
  function boot(){
    ...
    state.booted = true;
    status("Cargando lista de estudiantes...", "");
