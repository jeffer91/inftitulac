(function(window){
  ...
  function build(mesInicio, anioInicio, mesFin, anioFin){
    ...
    return { periodoId: label, periodoLabel: label, mesInicio: mi, anioInicio: ai, mesFin: mf, anioFin: af, estado: "ACTIVO", activo: true };
  }

  function guardar(periodo){
    var row = clean(periodo);
    return B.put(B.stores.periodos, row).then(function(){ B.cacheClear(); return row; });
  }
