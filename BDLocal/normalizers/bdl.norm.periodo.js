(function(window){
  ...
  var months = {
    ...
    julio:"07", jul:"07", agosto:"08", ago:"08", septiembre:"09", setiembre:"09", sep:"09", sept:"09",
    octubre:"10", oct:"10", noviembre:"11", nov:"11", diciembre:"12", dic:"12"
  };

  function extractYears(value){
    return T.text(value).match(/20\d{2}/g) || [];
  }

  function extractMonths(value){
    var found = [];
    T.searchKey(value).split(" ").forEach(function(part){
      var m = months[part] || "";
      if(m && found.indexOf(m) < 0){ found.push(m); }
    });
    return found;
  }
  ...
  function normalize(row, fallback){
    ...
    return {
      periodoId: id,
      periodoLabel: T.cleanSpaces(raw) || "Sin período",
      estado: revision ? "REVISION" : "ACTIVO",
      activo: !revision,
      requiereRevision: revision,
      motivo: revision ? "periodoId vacío o no detectado" : "",
      totalEstudiantes: 0,
